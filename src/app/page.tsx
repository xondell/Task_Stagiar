    ...
    <div className="theme-bg min-h-screen bg-neutral-950 text-neutral-100 font-sans selection:bg-lime-500 selection:text-black transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-6 py-16">
        <header className="mb-20 text-center space-y-6">
          ...
          <p className="theme-text-muted text-lg md:text-xl text-neutral-400 max-w-2xl mx-auto font-light leading-relaxed">
            {t.tagline}
          </p>
        </header>
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {documentTypes.map((doc: DocType) => (
            <Link
              ...
              key={doc.id}
              className="theme-card group relative rounded-3xl bg-white/5 border border-white/10 p-8 hover:bg-white/10 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-lime-500/20 overflow-hidden"
            >
              ...
              <div className="relative z-10 flex flex-col h-full space-y-5">
                ...
                <h3 className="theme-text-main text-2xl font-bold text-white group-hover:text-lime-300 transition-colors">{t.docTypes[doc.key]?.name ?? doc.name}</h3>
                <p className="theme-text-faint text-neutral-400 text-sm flex-grow leading-relaxed">{t.docTypes[doc.key]?.description ?? doc.description}</p>
